...
| **F3.6** | Motor de insights y alertas operativas | Panel de insights bajo el mapa + sistema de notificaciones sobre alertas de interés (eventos, cortes). Debe cumplir "avisa, no actúa" (`CLAUDE.md` §4) — ver `docs/investigacion/un backlog archivado` | `013`, `014` |
| **F4** | Contexto mediático | RSS de medios locales + GDELT filtrado por Valencia municipio (excluye Comunitat/Venezuela) + Reddit como señal secundaria; geolocalización de titulares por distrito/barrio mediante matching de texto (sin IA); tendencia de términos más repetidos por hora/día | `009`, `023`, `025` |
...
| **F5** | Pulido y compartición | Estado en URL, geolocalización de usuario + "qué tengo más cerca" (implementado, desbloqueado por `ADR-002`), acceso protegido con usuario+PIN en dominio propio (gate propio en middleware — Vercel Hobby no ofrece password-protection nativa), PWA instalable + shell offline, UX móvil (chasis táctil), rendimiento, accesibilidad | `012`, `018`, `028`, `029` |
