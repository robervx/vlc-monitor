...
Sigue sin ser capa de mapa — mismo panel de `013`. Cada tarjeta de insight de correlación muestra, además del texto habitual, un chip por cada `fuenteSpec` que participó (ej. "004" + "008"), para que se vea de un vistazo qué dos señales se cruzaron sin tener que leer la descripción completa.
...
| `lluvia-mas-trafico-denso` | El insight `lluvia-intensa-prevista` (spec 016, ya en `013`) está activo para una franja horaria próxima **y** existe al menos un distrito con tramos en `congestionado`/`cortado` ahora mismo | `urgente` (sube desde `aviso`, refuerza en vez de duplicar) | No es una regla nueva desde cero — añade contexto de tráfico al insight de lluvia ya existente en vez de generar dos tarjetas sueltas que se tendría que cruzar mentalmente. |
