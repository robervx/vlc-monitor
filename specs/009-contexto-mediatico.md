...
> **Estado:** v1-v6 `Implemented` y en producción. **v6 (2026-09-16)**: se retiró el bucket
> "ocio y deporte" — solo sobrevive el deporte de fútbol — y se añadió caducidad por ítem
> + próxima actualización del panel. Ver §8.
...
- Cada feed es de un único medio (`site:`), así que la atribución es la etiqueta fija de la fuente. El tag `<source>` de Google es inconsistente ("Levante-EMV" unas veces, "levante-emv.com" otras) y no se usa; solo se recorta el sufijo `" - <Medio>"` del `<title>`.
...
El endpoint devuelve `{ items: ItemMediatico[], fresh: boolean, fuentesFallidas: string[] }`, `items` ya sin los `excluido`, ordenados por `publicadoEn` desc, cap a 40. Dedup en 3 pasadas (`src/services/dedup-noticias.ts`): (1) URL exacta; (2) titular normalizado idéntico; (3) **misma noticia VA/ES del mismo medio** — Levante-EMV publica cada pieza en los dos idiomas con URL y titular distintos; se comparan los tokens significativos (≥5 letras, sin stopwords) con match difuso por prefijo de 5 / distancia de edición ≤ 2 (cognados: "Mercat/Mercado Central", "climatització/climatización"), y si ≥ 3 tokens casan y cubren ≥ 60% del titular más corto, es la misma. Cuando hay par, se conserva la versión en **castellano** (heurística de marcadores léxicos valencianos).
...
- [x] Feeds Google News (Levante-EMV, Cadena SER verificados en vivo con ítems reales; À Punt responde pero con volumen bajo — es un medio autonómico); links de redirección de `news.google.com` servidos tal cual (§2.2); atribución = etiqueta fija del feed, sufijo `" - Medio"` recortado del `<title>`.
...
- **Resuelto (v5.1):** el dedup VA/ES de un mismo medio (Levante-EMV publica todo en los dos idiomas) — 3ª pasada de `dedup-noticias.ts`, ver §4. Residual aceptado: si los dos titulares comparten menos de 3 tokens cognados (traducción muy libre) no se colapsan.
...
| 5.1 | 2026-09-03 | Dedup cross-idioma VA/ES del mismo medio (`src/services/dedup-noticias.ts` + 8 tests): Levante-EMV publica cada noticia en los dos idiomas; se colapsan por tokens cognados y se conserva la versión en castellano. De paso: la atribución de los feeds de Google News pasa a ser la etiqueta fija del feed (`<source>` devolvía a veces el dominio "levante-emv.com"). 276/276 tests, verificado en vivo (los 3 pares VA/ES del día se colapsan a la versión ES). |
...
| 6 | 2026-09-16 | **Filtro (`src/services/filtro-ambito-ciudad.ts`)**: nuevo marcador `MARCADORES_FUTBOL` (club/competición/estadio — deliberadamente sin vocabulario genérico de crónica tipo "fichaje"/"lesión", compartido entre deportes, para no dar falsos positivos con el Valencia Basket). El deporte que no es fútbol se excluye **antes** que cualquier señal positiva (distrito/hito) — decisión explícita del usuario: "solo interesa cuando hay partido de fútbol" gana incluso sobre una mención de ciudad real. La categoría `'ocio'` desaparece (`CategoriaMediatica` pasa a `'general' \| 'deporte'`); las fuentes temáticas de ocio (Valencia Secreta/Bonita) se tratan como cualquier otra fuente de ciudad — se retira el campo ya sin uso `categoriaFuente` de `EntradaClasificacion`/`FiltroFuenteConfig` (`mediatico.ts`). **UI (`src/main.ts`)**: se retira el toggle "Ocio y deporte" y el bucket oculto correspondiente — el deporte que sobrevive el filtro (ya garantizado fútbol) se funde en el flujo normal de distrito/ciudad/general, sin sección aparte. Cada ítem se marca `caducado` (opacidad reducida + aviso) si lleva más de 24h publicado (`UMBRAL_CADUCADO_MS`); el meta del panel añade "próxima actualización en ~N min" a partir del mismo TTL que la caché del endpoint (15 min, sin importar código de servidor al bundle de cliente). Tests actualizados/nuevos en `filtro-ambito-ciudad.test.ts` (crónica de fútbol ya no se descarta; baloncesto/tenis/motor sí, incluso con distrito mencionado). 338/338 tests (3 nuevos), `npm run typecheck` verde, verificado en navegador (sin toggle, sin secciones "Ocio y cultura"/"Deporte", meta muestra "próxima actualización en ~15 min"). |
