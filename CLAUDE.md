...
**Actualización 2026-08-18 (ver `docs/decisiones/ADR-001-linea-producto-seguridad-publica.md`):** el producto pivota de audiencia — de panel abierto orientado al ciudadano a **herramienta de despacho para mandos de Policía Local de València**, bajo el nombre de marca **"Intelligent City Monitor"**. Es una única plataforma (no dos apps separadas): la pantalla principal ya construida (mapa + capas F0-F4) pasa a ser el marco base sobre el que se añade cabecera institucional (nombre + logo oficial, con autorización del cuerpo) y una barra lateral plegable de funcionalidades. Detalle completo en `docs/investigacion/GEMELO_DIGITAL_SEGURIDAD_PUBLICA_PROPUESTA.md`. Esto **no revive la arquitectura del antiguo CISE Command Center** (sigue sin SQL Server, sin módulos 1A/1B/2/3, sigue siendo TypeScript/Vite/MapLibre+deck.gl) — es la misma base técnica de VLC Monitor reorientada de audiencia. El límite ético/legal de §4 (agregado, sin biometría fuera de dispositivo, "avisa no actúa") se mantiene íntegro y es, si acaso, más estricto ahora que hay un usuario operativo real.

**Este NO es** el antiguo proyecto "CISE Command Center" (sala de llamadas policial, SQL Server, módulos 1A/1B/2/3). Ese pivote arquitectónico quedó documentado y superado en `docs/01_VIABILIDAD_VISION_Y_PROCESO.md` §3.6, y sigue siendo válido — el pivote de audiencia de arriba no es una vuelta a esa arquitectura. Si encuentras referencias sueltas a "situación operativa", "CISE" o módulos 1A/1B/2/3 en algún fichero antiguo, son legado — no las extiendas ni las tomes como guía de producto.
