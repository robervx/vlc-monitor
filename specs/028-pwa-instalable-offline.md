...
Derivados del logo ya presente (`public/assets/logo-anterior.png`) sobre fondo navy `#0b1f33`, con zona segura para la variante `maskable` (el escudo ocupa ≤ 80 % del lienzo). Generados una vez y versionados — no hay pipeline en runtime.
...
Los iconos (`icon-192.png`, `icon-512.png`, `icon-maskable-512.png`, `apple-touch-icon.png`) se generan del logo con `npm run iconos:pwa` (`scripts/generar-iconos-pwa.ts`: `sips` para el reescalado + aplanado del alfa contra el navy en Node puro) y se **versionan** en `public/icons/`. No es una dependencia de build.
...
- [x] Iconos generados del logo (`npm run iconos:pwa`), fondo navy y zona segura para el `maskable` — verificados visualmente. Versionados en `public/icons/`.
