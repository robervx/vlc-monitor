...
| `020` | Grafo viario base (nodos + tramos), infraestructura reutilizable | F8 | **Implemented** — recorte de la Opción B de `ADR-002`, sin simulador. Overpass+TS (no Geofabrik/osmnx, ver spec §2), índice `rbush` (no H3). 9.193 nodos/13.225 tramos generados y verificados con 5 ubicaciones reales; endpoint `GET /api/grafo-viario/v1/tramos` verificado. CDNCV verificado como fuente, resolución de nombre pendiente (ver spec §7) | `000` |
| `021` | Motor de propuesta de cordón de seguridad por incidente | F8 | **Draft** (deliberado, no `Approved`) — DoD de software completo: motor de cálculo por distancia de red, sección en el sidebar con formulario/badges/guard PII, verificado con typecheck/test (146/146) y en navegador. **Sigue bloqueada para `Approved`**: falta contraste real de la tabla de perímetros con el el servicio de bomberos competente (acción humana, no de software) | `020`, `019` |
