...
| Histórico/correlación de señales | Postgres serverless (Neon/Supabase, Vercel Marketplace, free tier) — append-only, junto a Redis, no en su lugar | Redis sirve el último valor con TTL; no correlaciona bien varias dimensiones (calle+clima+incidencia+evento) a lo largo del tiempo. Ver `docs/decisiones/ADR-006-almacen-historico-senales.md`. |
