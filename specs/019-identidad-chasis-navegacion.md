...
**Activo de marca**: logo del proyecto, aportado directamente por el propietario del producto con autorización confirmada del cuerpo (registrado en ADR-002) — no obtenido de ninguna búsqueda ni fuente externa. Fichero esperado en `public/assets/logo-anterior.png`.
...
- [x] Cabecera visible con el nombre "Intelligent City Monitor" y el logo del proyecto — fichero recibido y colocado en `public/assets/logo-anterior.png`; `<img>` con `onerror` que oculta el logo sin romper la cabecera si el fichero llegara a faltar.
...
- [x] Atribución "Uso interno — no es un servicio público" visible en el pie del sidebar; tagline del proyecto en la cabecera (v3: se retira "— herramienta interna" de la tagline por decisión del usuario, queda solo en el pie del sidebar).
