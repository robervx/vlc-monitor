...
**Nota de estado (v2, 2026-08-25)**: el DoD de software (§6) está completo e implementado — motor de cálculo, UI, tests, badges de confianza, guard de datos personales. **Sigue en `Draft`, deliberadamente no `Approved`**: esta spec fija su propio bloqueante (§7) de que `Approved` requiere contraste real con el el servicio de bomberos competente, que es una acción humana fuera de esta sesión — construir el software no sustituye esa validación, y marcar esto como algo más que `Draft` sería exactamente el riesgo que el badge de confianza está diseñado para evitar.

## 0. Contexto de la decisión

Extiende la línea aprobada en `ADR-002` (Opción C). No es el simulador de tráfico (Fase 2 de `docs/investigacion/un documento de diseño archivado`, que sigue fuera de alcance) — es un problema más simple: dado un incidente puntual, proponer un perímetro y qué tramos cortar, usando el grafo de spec `020` y una tabla de reglas **basada en fuentes reales**, no inventada.
...
**Reparto de roles real (aclarado explícitamente por el usuario, 2026-08-25):** en un incendio, gestión municipal suele ser la primera en llegar, antes o a la vez que Bombers — y mientras Bombers se dedica por completo a la extinción, es la autoridad local quien gestiona el perímetro, el corte de calles y el desvío de tráfico. Esta herramienta es la guía de **ese** trabajo — el de la autoridad local como primer interviniente gestionando el entorno — no una herramienta para que Bombers decida cómo atacar el fuego. Por eso el destinatario del formulario y de la propuesta es el agente/responsable municipal en el sitio, no el servicio de extinción.

Hoy esa gestión de perímetro sale solo del criterio y la experiencia de quien lo coordina, sin ninguna ayuda visual del entorno viario real. El sistema **propone**, nunca decide ni notifica por sí solo (`CLAUDE.md` §4) — y precisamente porque quien gestiona el perímetro aquí no es criterio del servicio de extinción, es todavía más importante que las cifras de esta spec no se lean como si vinieran del criterio técnico de extinción: son un punto de partida de apoyo al corte de calles, siempre editable y pendiente de contraste real (ver §7).
...
**Hueco identificado, explícito y no resuelto por esta investigación**: ni la normativa española revisada ni la guía UK dan una cifra oficial para (a) incendio de vehículo eléctrico en plena calle (fuera de aparcamiento) ni (b) perímetro de vivienda/edificio por número de plantas o intensidad. Ambos casos se modelan en la sección 3 como **radio de partida editable, marcado como estimación**, no como dato normativo — y quedan anotados en la sección 7 como pendientes de contraste con el el servicio de bomberos competente antes de que esta spec pueda pasar a `Approved`.
...
- **Bloqueante para pasar a `Approved`**: contraste de la tabla completa (en particular vehículo eléctrico en vía pública y perímetro de vivienda/edificio por plantas) con el el servicio de bomberos competente o el servicio competente — no se resuelve solo con búsqueda documental, tal como exige `CLAUDE.md` §8.2 para fuentes no verificadas con una llamada/contacto real.
