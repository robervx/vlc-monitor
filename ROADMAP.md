...
| **F3** | Pulso de Distrito | v3: índice compuesto (tráfico + aire + meteo + incidencias), inspirado en el CII de World Monitor. **v4 (Draft):** se abandona el índice ponderado por un catálogo de escenarios de conjunción con localización por calle — cuando se cruzan tráfico denso + lluvia inminente, o incidencia + tráfico denso co-localizados, salta un aviso de proactividad. Consolidado con el motor de insights (F3.6) | `010` (depende de `001`, `002`, `004`, `008`, `016`, `026`; prerrequisito `017` v4) |
| **F3.5** | Agenda y aglomeraciones previsibles | Eventos culturales (Fallas), calendario deportivo, ferias — el mejor predictor legal de aglomeraciones; incidencias oficiales de vía pública (obras/cortes/festejos, dato real del Geoportal); agenda general de eventos culturales vía scraping resiliente de valencia.es; avisos oficiales de movilidad — incidencias y previsiones con detalle de corte, calle y plano, de la propia página del área de Movilidad | `008`, `026`, `027`, `048` |
| **F3.6** | Motor de insights y alertas operativas | Panel de insights bajo el mapa + alertas emergentes (toast al aparecer una alerta nueva, spec `013` v4a) + dashboard de indicadores (KPIs, spec `034`) + sistema de notificaciones sobre alertas de interés. Debe cumplir "avisa, no actúa" (`CLAUDE.md` §4) | `013`, `014`, `034` |
...
| **F5** | Pulido y publicación | Estado en URL, geolocalización de usuario + "qué tengo más cerca", acceso protegido opcional con usuario+PIN (gate propio en middleware, fail-open), PWA instalable + shell offline, UX móvil (chasis táctil), jerarquía de capas en el selector (estado en tiempo real vs contexto), re-marca genérica + repo público MIT (ADR-002), rendimiento, accesibilidad | `012`, `018`, `028`, `029`, `030`, `033`, `035`, `036` |
...
- Globo 3D, app de escritorio, monetización, la **API** de pago de X/Twitter (su widget gratuito de embebido sí se usa, ver `039`).
