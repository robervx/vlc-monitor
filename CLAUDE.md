...
**Actualización 2026-08-29 (ver `docs/decisiones/ADR-002-repo-publico-marca-generica.md`):** este repo se publica como monitor de datos abiertos de propósito general. Se retiró el logo del proyecto y toda identidad institucional que introdujo ADR-002 (que orientaba el producto a "herramienta de gestión para responsables municipales"). **ADR-002 revierte esa decisión de audiencia.** Las funcionalidades que se construyeron bajo ADR-002 —cordón de incidente (`021`), simulador de cortes de calle (`022`), motor de insights (`013`/`024`), grafo viario (`020`)— **se quedan**: son herramientas genéricas de gestión municipal (obras, eventos, emergencias), solo se les quitó el lenguaje de un caso de uso concreto. El límite ético/legal de §4 se mantiene **íntegro** y aplica con más razón a un proyecto público. Cualquier despliegue que quiera orientación operativa específica la añade por su cuenta, fuera de este repo, con su propia autorización y cumplimiento.

**Este NO es** el antiguo proyecto un proyecto anterior distinto. Ese pivote arquitectónico quedó documentado y superado en `docs/01_VIABILIDAD_VISION_Y_PROCESO.md` §3.6. Si encuentras referencias sueltas a "situación operativa", "proyecto anterior", "gestión operativa" o módulos 1A/1B/2/3 en algún fichero antiguo, son legado — no las extiendas ni las tomes como guía de producto.
