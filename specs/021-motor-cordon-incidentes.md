...
**Nota de estado (v2, 2026-08-25)**: el DoD de software (§6) está completo e implementado — motor de cálculo, UI, tests, badges de confianza, guard de datos personales. En su momento se mantuvo deliberadamente en `Draft` porque esta spec se había fijado a sí misma un bloqueante (§7): que `Approved` requería contraste real con el servicio de bomberos competente antes de poder darse por cerrada.

**Nota de estado (v4, 2026-09-16) — bloqueante retirado por decisión de producto**: el criterio de §7 partía de tratar el cordón como si necesitara validación técnica de bomberos antes de poder usarse. El usuario (propietario del producto) ha corregido ese planteamiento: la herramienta no sustituye ni anticipa el criterio de bomberos — es una **primera aproximación de apoyo, rápida y editable, para el momento de confusión inicial cuando la policía local es el primer cuerpo en llegar al incidente**. Cuando llegan bomberos u otros servicios, ajustan el perímetro con su propio criterio en su PMA (puesto de mando avanzado), sin que este software se lo impida ni necesite su visto bueno previo — exactamente el mismo principio de "avisa, no actúa" que ya aplicaba (`CLAUDE.md` §4): propone, queda siempre editable, nunca decide. Con esto no queda ningún bloqueante pendiente y el software ya estaba completo (§6) — la spec pasa a `Implemented`.
...
El bloqueante de §7 (contraste de la tabla de perímetros con el servicio de bomberos competente) quedó retirado en v4 (ver nota de estado arriba) — v3 en sí no cambió el estado `Draft`, pero la spec ya no depende de esa condición.

## 0. Contexto de la decisión

Herramienta de gestión municipal sobre el grafo viario (`020`). No es un simulador de tráfico completo (fuera de alcance) — es un problema más simple: dado un incidente puntual, proponer un perímetro y qué tramos cortar, usando el grafo de spec `020` y una tabla de reglas **basada en fuentes reales**, no inventada.
...
**Destinatario:** el responsable municipal que gestiona el perímetro, el corte de calles y el desvío de tráfico en el sitio de un incidente. No es una herramienta para el servicio de extinción, que se ocupa del fuego.

Hoy esa gestión de perímetro sale solo del criterio y la experiencia de quien la coordina, sin ninguna ayuda visual del entorno viario real. El sistema **propone**, nunca decide ni notifica por sí solo (`CLAUDE.md` §4). Las cifras de esta spec son un punto de partida de apoyo al corte de calles, siempre editable y pendiente de contraste real (ver §7); no deben leerse como criterio técnico de extinción.
...
| Guías UK "National Operational Guidance" (incendio estructural) | Confirma que el tamaño del cordón en incendio de edificio **no tiene tabla fija** — depende de altura, viento, riesgo de desprendimiento, valorado en el sitio | Incendio estructural general | ⚠️ Confirma la ausencia de cifra única — no aporta un número, aporta la razón por la que no debe inventarse uno |

**Hueco identificado, explícito y no resuelto por esta investigación**: ni la normativa española revisada ni la guía UK dan una cifra oficial para (a) incendio de vehículo eléctrico en plena calle (fuera de aparcamiento) ni (b) perímetro de vivienda/edificio por número de plantas o intensidad. Ambos casos se modelan en la sección 3 como **radio de partida editable, marcado como estimación**, no como dato normativo — quedan anotados en la sección 7 como huecos de fuente conocidos y deliberadamente conservadores (no como bloqueante de `Approved`, ver nota de estado v4).
...
interface PropuestaCordon {
  ...
  generadaEn: string;
  editadaManualmente: boolean;           // true en cuanto se ajusta algo — trazabilidad de que ya no es la propuesta cruda
}
```
...
- **Ya no es un bloqueante de `Approved`/`Implemented` (decisión de producto, v4, 2026-09-16)**: el hueco de fuente para vehículo eléctrico en vía pública y perímetro de vivienda/edificio por plantas sigue siendo real (§2) y las cifras siguen marcadas como estimación conservadora con badge de confianza — pero la herramienta se entiende como primera aproximación editable para el momento de llegada de la policía, no como criterio técnico de bomberos, así que no necesita su validación previa para poder usarse. Si en algún momento se dispone de un contraste real con el servicio de bomberos, se incorpora como una mejora de confianza de las reglas (subir `confianza` de `estimacionPendienteValidar` a `oficialVerificada`), no como condición para que la herramienta exista.
...
| 4 | 2026-09-16 | **Decisión de producto, sin cambio de software**: se retira el bloqueante de §7 (contraste previo con bomberos) — la herramienta se re-encuadra explícitamente como primera aproximación de apoyo para cuando la policía local llega primero al incidente, editable, sin pretender sustituir el criterio de otros cuerpos que ajustan después en su propio PMA. El DoD de software (§6) ya estaba completo desde v2. Spec pasa de `Draft` a `Implemented`. |
