...
**Relación con ADR-002:** **revierte la decisión de audiencia** de ADR-002 (Opción C — "herramienta de gestión para responsables municipales"). **Mantiene íntegro** el límite ético/legal de `CLAUDE.md` §4, que aplica igual o con más razón a un proyecto público.

---

## Contexto

ADR-002 fijó el producto como herramienta de gestión para gestión municipal, con identidad institucional (nombre de marca + logo, con autorización del cuerpo). Sobre esa base se construyeron el chasis (spec `019`), la geolocalización de campo (`012`), el grafo viario (`020`), el motor de cordón (`021`), el simulador de cortes (`022`) y el motor de insights v2 (`024`).
...
2. **Retirar el logo del proyecto** y toda marca institucional. La autorización del cuerpo cubría un uso interno, no una publicación abierta.
...
- Spec `019` v4: la nota de procedencia del asset de marca cambia (ya no es "logo con autorización" sino "placeholder neutro, sustituible").
- Spec `018` v3: el middleware pasa de fail-closed a fail-open cuando no hay `AUTH_SECRET`.
- ADR-002 queda como registro histórico: su decisión de audiencia está revertida por este ADR; su aportación viva es haber desbloqueado specs `012` y `020`-`022`, que siguen válidas como herramientas genéricas.
- Los documentos de investigación con enfoque de un caso de uso concreto (`un documento de diseño archivado`) se conservan como contexto histórico; no son guía de producto del repo público.
