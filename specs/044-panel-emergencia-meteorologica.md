...
| Lluvia por zona (candidato fuerte, añadido 2026-09-17) | **AVAMET** (Associació Valenciana de Meteorologia) — tabla de precipitación en vivo por estación, `avamet.org/mxo-mxo-prec.php?territori=c15` (territori `c15` = comarca de l'Horta/València) | **Verificado con petición real**: HTTP 200, `robots.txt` permisivo (solo `Disallow: /_mxarxa/` y `/_gestor/`). La tabla (`id="mxoCompletaN_0"`) trae, por estación, precipitación de hoy/mes/año, intensidad, tipo de lluvia y acumulados a 1/4/8/12/24/48/72 h, agrupadas por comarca, actualizadas cada pocos minutos (confirmado "16 estaciones operativas" con timestamp real al consultar). Estaciones vistas dentro de término de València: "Alqueries del Pelut (Carpesa)", "Playa de la Garrofera (el Saler)", "Albors", "Altocúmulo", cada una con id propio (`c15m250e39`, etc.) y página de detalle en `mxo_i.php?id=<id>`. | AVAMET ya es fuente del proyecto (spec `039`, widgets institucionales), así que reutilizar su dato de pluviometría es coherencia añadida, no una fuente nueva sin precedente. **Pendiente antes de congelar contrato**: no se ha localizado todavía el lat/lon de cada estación (necesario para "cada dato en su zona" sobre el mapa) — ni la tabla ni una comprobación rápida de `mxo_i.php?id=c15m250e39` lo trajeron; puede hacer falta geocodificar por el topónimo de cada estación o buscar un endpoint/KML de estaciones aparte. Superaría a Open-Meteo en fidelidad (estaciones reales, no modelo) si esto se resuelve. |
