...
| Tiles de OpenFreeMap (`*.openfreemap.org`) | **Sin regla — no interceptar** (v3) | Cachear teselas vectoriales en el SW (respuestas opacas `status 0`, rangos, clones) es un footgun conocido de Workbox y hay incidencias documentadas de MapLibre GL con SW de por medio. Como el offline de teselas es marginal aquí ("solo lo ya visitado"), se deja el mapa base en red directa. Offline de teselas = fast-follow con verificación en navegador real. |
...
- [x] Iconos generados (`npm run marca`), fondo navy y zona segura para el `maskable` — verificados visualmente. Versionados en `public/icons/`. (v2 usaba el logo; spec `030` lo sustituyó por el logo neutro.)
- [x] Rutas del SW correctas en `dist/sw.js`: `NetworkFirst` para navegación (`icm-shell`), `StaleWhileRevalidate` para `/api/**/v1/**` (`icm-datos`, solo `200`), `NetworkOnly` para `/api/auth/*`. OpenFreeMap **sin regla** (v3 — ver §3): las teselas van en red directa, no por el SW.
...
| 3 | 2026-08-31 | Se elimina la regla `runtimeCaching` de `openfreemap.org` (era `CacheFirst` con `statuses: [0, 200]`). Motivo: cachear teselas vectoriales en el SW —respuestas opacas incluidas— es un footgun de Workbox con incidencias documentadas en MapLibre GL, y el offline de teselas aquí es marginal. Nota: durante la depuración se sospechó que esta regla dejaba el mapa base en blanco, pero la causa real de que el mapa no pintara en las pruebas era el navegador de la sesión de Claude Code, que mantiene `document.visibilityState = "hidden"` y con ello el bucle `requestAnimationFrame` de MapLibre parado (forzando `_render()` a mano el mapa pinta correcto). En un navegador de usuario (pestaña visible) el mapa base renderiza sin problema. |
